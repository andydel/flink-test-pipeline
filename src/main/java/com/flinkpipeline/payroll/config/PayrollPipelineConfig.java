...
/**
 * Comprehensive configuration management for the Payroll Data Quality Pipeline.
 * Supports multiple configuration sources (properties files, environment variables, command line),
 ...
 * 5. Default configuration files (application.properties)
 * 6. Built-in defaults
 */
public class PayrollPipelineConfig {
  ...
  // Constructor
  public PayrollPipelineConfig(Environment environment,
    ...
    LOG.info("Initialized PayrollPipelineConfig for environment: {}, version: {}", environment, version);
  }
  ...
  public static PayrollPipelineConfig fromArgs(String[] args) throws IOException {
    ...
    Environment environment = Environment.valueOf(
        argMap.getOrDefault("environment",
            System.getProperty("payroll.environment",
   ...
   */
  private static PayrollPipelineConfig loadConfiguration(Environment environment,
   ...
   */
  private static Properties loadPropertiesWithHierarchy(Environment environment) throws IOException {
  ...
  private static void loadPropertiesFromResource(Properties properties, String resourceName) {
    try (InputStream input = PayrollPipelineConfig.class.getClassLoader().getResourceAsStream(resourceName)) {
      if (input != null) {
  ...
  private static void applyEnvironmentVariables(Properties properties) {
    System.getenv().forEach((key, value) -> {
      if (key.startsWith("PAYROLL_")) {
        String propertyKey = key.toLowerCase().replace("payroll_", "payroll.").replace("_", ".");
   ...
   */
  private static PayrollPipelineConfig buildConfiguration(Environment environment, Properties properties) {
    String applicationName = properties.getProperty("payroll.application.name", "payroll-data-quality-pipeline");
  ...
  private static ExecutionConfig buildExecutionConfig(Properties properties) {
    return new ExecutionConfig(
        ...
        Duration.ofMillis(Long.parseLong(properties.getProperty("payroll.execution.buffer.timeout", "100"))),
  ...
  private static KafkaConfig buildKafkaConfig(Properties properties) {
    return new KafkaConfig(
        ...
        Arrays.asList(properties.getProperty("payroll.kafka.topics", "payroll-employees").split(",")),
        ...
        Duration.ofSeconds(Long.parseLong(properties.getProperty("payroll.kafka.consumer.timeout", "30")))
    );
  }
  ...
  private static ValidationConfig buildValidationConfig(Properties properties) {
    return new ValidationConfig(
        ...
        Boolean.parseBoolean(properties.getProperty("payroll.validation.duplicate.detection.enabled", "true")),
        Duration.ofMinutes(Long.parseLong(properties.getProperty("payroll.validation.duplicate.window.minutes", "60"))),
        Boolean.parseBoolean(properties.getProperty("payroll.validation.compliance.auditing.enabled", "true")),
        Boolean.parseBoolean(properties.getProperty("payroll.validation.strict.mode.enabled", "false"))
    );
  }
  ...
  private static IcebergConfig buildIcebergConfig(Properties properties) {
    return new IcebergConfig(
        ...
        Duration.ofMinutes(Long.parseLong(properties.getProperty("payroll.iceberg.compaction.interval.minutes", "30")))
    );
  }

  private static HrWorkflowConfig buildHrWorkflowConfig(Properties properties) {
    return new HrWorkflowConfig(
        Duration.ofHours(Long.parseLong(properties.getProperty("payroll.hr.sla.threshold.hours", "4"))),
        ...
        Duration.ofHours(Long.parseLong(properties.getProperty("payroll.hr.escalation.threshold.hours", "6"))),
  ...
  private static SecurityConfig buildSecurityConfig(Properties properties) {
    return new SecurityConfig(
        Boolean.parseBoolean(properties.getProperty("payroll.security.pii.encryption.enabled", "false")),
        ...
        Boolean.parseBoolean(properties.getProperty("payroll.security.authentication.enabled", "false"))
    );
  }
  ...
  private static MetricsConfig buildMetricsConfig(Properties properties) {
    return new MetricsConfig(
        ...
        Duration.ofSeconds(Long.parseLong(properties.getProperty("payroll.metrics.interval.seconds", "60"))),
  ...
  private static HealthCheckConfig buildHealthCheckConfig(Properties properties) {
    return new HealthCheckConfig(
        ...
        Duration.ofSeconds(Long.parseLong(properties.getProperty("payroll.health.check.interval.seconds", "30")))
    );
  }
  ...
  private static StateConfig buildStateConfig(Properties properties) {
    return new StateConfig(
        ...
        Boolean.parseBoolean(properties.getProperty("payroll.state.incremental.checkpoints", "true")),
        Long.parseLong(properties.getProperty("payroll.state.ttl.milliseconds", "86400000")) // 1 day
    );
  }
  ...
  private static Map<String, HRWorkflowRoutingOperator.HRTeamConfig> getDefaultHrTeamConfig() {
    ...
    config.put("CRITICAL", new HRWorkflowRoutingOperator.HRTeamConfig(
        Arrays.asList("hr.manager@company.com", "hr.senior.analyst@company.com"), 5));
    config.put("HIGH", new HRWorkflowRoutingOperator.HRTeamConfig(
        Arrays.asList("hr.analyst1@company.com", "hr.analyst2@company.com"), 10));
    config.put("MEDIUM", new HRWorkflowRoutingOperator.HRTeamConfig(
        Arrays.asList("hr.coordinator1@company.com", "hr.coordinator2@company.com"), 15));
    config.put("LOW", new HRWorkflowRoutingOperator.HRTeamConfig(
        Arrays.asList("hr.assistant@company.com"), 20));
  ...
  public void validate() throws IllegalArgumentException {
    ...
    if (checkpointConfig.isEnabled() && (checkpointConfig.getStoragePath() == null ||
                                        checkpointConfig.getStoragePath().isEmpty())) {
      throw new IllegalArgumentException("Checkpoint storage path must be specified when checkpointing is enabled");
    }
  ...
  public IcebergConfig getIcebergConfig() { return icebergConfig; }
  public HrWorkflowConfig getHrWorkflowConfig() { return hrWorkflowConfig; }
  public SecurityConfig getSecurityConfig() { return securityConfig; }
  public CheckpointConfig getCheckpointConfig() { return checkpointConfig; }
  ...
  public String toString() {
    return String.format("PayrollPipelineConfig{environment=%s, application=%s, version=%s}",
