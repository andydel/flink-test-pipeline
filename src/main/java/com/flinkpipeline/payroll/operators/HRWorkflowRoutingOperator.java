...
/**
 * Apache Flink operator for intelligent routing of failed payroll records to appropriate HR workflow queues.
 * Implements priority-based routing, SLA tracking, escalation logic, and HR workload balancing.
 *
 * Features:
 * - Priority-based routing (CRITICAL, HIGH, MEDIUM, LOW)
 * - Automatic escalation for SLA violations
 * - HR team workload balancing and round-robin assignment
 * - Retry logic for transient failures
 * - Compliance audit logging for all routing decisions
 * - Real-time metrics and dashboard integration
 * - Integration with external HR systems (HRIS, ticketing)
 */
public class HRWorkflowRoutingOperator extends KeyedProcessFunction<String, FailedPayrollRecord, FailedPayrollRecord> {
  ...
  private static final Map<String, Duration> SLA_THRESHOLDS = Map.of(
  ...
  public HRWorkflowRoutingOperator(Duration slaThreshold,
  ...
  public void open(Configuration parameters) throws Exception {
    ...
    LOG.info("Initializing HRWorkflowRoutingOperator with SLA threshold: {}, max retries: {}",
             slaThreshold, maxRetryAttempts);
  ...
  @Override
  public void processElement(FailedPayrollRecord record, Context context,
                           Collector<FailedPayrollRecord> out) throws Exception {

    totalRecordsRouted.incrementAndGet();
    String recordKey = generateRecordKey(record);

    LOG.debug("Processing failed record for routing: ID={}, Priority={}",
             record.getOriginalRecord().getEmployeeId(), record.getCorrectionPriority());
    ...
    try {
      ...
      LOG.debug("Successfully routed record to {} queue: ID={}",
               routingPriority, record.getOriginalRecord().getEmployeeId());

    } catch (Exception e) {
      LOG.error("Failed to route payroll record: ID={}",
               record.getOriginalRecord().getEmployeeId(), e);
  ...
  @Override
  public void onTimer(long timestamp, OnTimerContext context, Collector<FailedPayrollRecord> out) throws Exception {
    String recordKey = context.getCurrentKey();
    FailedRecordState state = recordState.get(recordKey);

    if (state != null) {
      Duration timeSinceCreation = Duration.between(state.getCreationTime(), Instant.ofEpochMilli(timestamp));

      // Check for SLA violation
      String priority = state.getFailedRecord().getCorrectionPriority();
      ...
      if (timeSinceCreation.compareTo(slaLimit) > 0) {
        ...
        LOG.warn("SLA violation detected for record: ID={}, Priority={}, Age={}",
                state.getFailedRecord().getOriginalRecord().getEmployeeId(),
                priority, timeSinceCreation);
  ...
  private String determineRoutingPriority(FailedPayrollRecord record, FailedRecordState state) {
    String currentPriority = record.getCorrectionPriority();
   ...
   */
  private void routeToEscalatedQueue(FailedPayrollRecord record, FailedRecordState state, Context context) {
    ...
    LOG.warn("Escalated record to management queue: ID={}, Retries={}",
            record.getOriginalRecord().getEmployeeId(), state.getRetryCount());
  }
  ...
  private void routeToCriticalQueue(FailedPayrollRecord record, String reason, Context context) {
    FailedPayrollRecord criticalRecord = FailedPayrollRecord.builder()
        .from(record)
   ...
   */
  private FailedPayrollRecord enrichRecordForRouting(FailedPayrollRecord record,
                                                    FailedRecordState state,
                                                    String priority) throws Exception {

    FailedPayrollRecord.Builder builder = FailedPayrollRecord.builder().from(record);

    // Update priority
    builder.correctionPriority(priority);

    // Add retry information
    if (state.getRetryCount() > 0) {
      String retryInfo = String.format("Retry #%d of %d", state.getRetryCount(), maxRetryAttempts);
      builder.hrCorrectionInstructions(
          (record.getHrCorrectionInstructions() != null ? record.getHrCorrectionInstructions() + " | " : "") +
          retryInfo);
    }

    // Assign HR team member if load balancing is enabled
    if (enableLoadBalancing) {
      String assignedTeamMember = assignHRTeamMember(priority);
      if (assignedTeamMember != null) {
        builder.hrCorrectionInstructions(
            (builder.build().getHrCorrectionInstructions() != null ?
             builder.build().getHrCorrectionInstructions() + " | " : "") +
    ...
    builder.hrCorrectionInstructions(
        (builder.build().getHrCorrectionInstructions() != null ?
         builder.build().getHrCorrectionInstructions() + " | " : "") +
        "SLA Deadline: " + deadline.toString());
   ...
   */
  private FailedPayrollRecord enrichRecordForEscalation(FailedPayrollRecord record, FailedRecordState state) {
    return FailedPayrollRecord.builder()
        .from(record)
        .correctionPriority("ESCALATED")
        .hrCorrectionInstructions(
            "ESCALATED: Record failed " + state.getRetryCount() + " routing attempts. " +
            "Age: " + Duration.between(state.getCreationTime(), Instant.now()).toString() + ". " +
  ...
  private void retryRouting(FailedRecordState state, OnTimerContext context) throws Exception {
    ...
    String newPriority = escalatePriority(state.getFailedRecord().getCorrectionPriority());
    FailedPayrollRecord retryRecord = enrichRecordForRouting(state.getFailedRecord(), state, newPriority);

    routeToHRQueue(retryRecord, newPriority, context);
    LOG.info("Retrying routing for record: ID={}, Attempt={}",
            state.getFailedRecord().getOriginalRecord().getEmployeeId(), state.getRetryCount());
  }

  /**
   * Generate audit logs for routing decisions
   */
  private void generateRoutingAuditLog(FailedPayrollRecord record, String priority, Context context) {
    ComplianceAuditLog auditLog = ComplianceAuditLog.createDataProcessingAudit(
   ...
   */
  private void generateSLAViolationAuditLog(FailedPayrollRecord record, Duration age, OnTimerContext context) {
    ComplianceAuditLog violationLog = ComplianceAuditLog.createComplianceViolationAudit(
        record.getOriginalRecord().getEmployeeId(),
        "SLA_VIOLATION",
        "HR workflow SLA violation - record age: " + age.toString(),
        "HIGH"
    );
  ...
  private boolean hasRegulatoryViolations(FailedPayrollRecord record) {
    return record.getValidationErrors().stream()
        .anyMatch(error -> error.toLowerCase().contains("regulatory") ||
  ...
  private boolean hasPIIViolations(FailedPayrollRecord record) {
    return record.getValidationErrors().stream()
        .anyMatch(error -> error.toLowerCase().contains("ssn") ||
  ...
  private boolean impactsPayrollProcessing(FailedPayrollRecord record) {
    return record.getValidationErrors().stream()
        .anyMatch(error -> error.toLowerCase().contains("wage") ||
  ...
  private String escalatePriority(String currentPriority) {
    switch (currentPriority) {
      ...
      case "CRITICAL": return "CRITICAL"; // Already at highest priority
  ...
  private static Map<String, HRTeamConfig> getDefaultHRTeamConfig() {
    ...
    config.put("CRITICAL", new HRTeamConfig(Arrays.asList(
        "hr.manager@company.com", "hr.senior.analyst@company.com")));
    config.put("HIGH", new HRTeamConfig(Arrays.asList(
        "hr.analyst1@company.com", "hr.analyst2@company.com", "hr.specialist@company.com")));
    config.put("MEDIUM", new HRTeamConfig(Arrays.asList(
        "hr.coordinator1@company.com", "hr.coordinator2@company.com")));
    config.put("LOW", new HRTeamConfig(Arrays.asList(
        "hr.intern@company.com", "hr.assistant@company.com")));
   ...
   */
  public static class FailedRecordState {
    ...
    public FailedPayrollRecord getFailedRecord() { return failedRecord; }
    public Instant getCreationTime() { return creationTime; }
    public int getRetryCount() { return retryCount; }
    public void incrementRetry() { this.retryCount++; }
  }

  public static class HRWorkloadState {
  ...
  public static class HRTeamConfig {
    ...
    public int getMaxConcurrentCases() { return maxConcurrentCases; }
  }
  ...
  public static class RoutingStatistics {
    ...
    public RoutingStatistics(long totalRouted, long criticalRecords, long escalatedRecords,
                           long slaViolations, Map<String, Long> queueCounts) {
    ...
    public long getCriticalRecords() { return criticalRecords; }
    public long getEscalatedRecords() { return escalatedRecords; }
