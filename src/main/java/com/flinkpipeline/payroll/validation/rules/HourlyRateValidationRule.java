...
/**
 * Hourly rate wage compliance validation rule implementation (DQ-007). Validates hourly wage rates
 * against federal minimum wage and executive compensation limits. Enforces federal employment law
 * compliance for wage requirements.
 */
public class HourlyRateValidationRule {
    ...
    try {
      ...
      if (hourlyRateCents == null) {
        return createFailureResult(
            "Hourly rate is required and cannot be null",
            "Enter hourly rate between "
                + formatCurrency(MINIMUM_WAGE_CENTS)
                + " and "
                + formatCurrency(MAXIMUM_WAGE_CENTS),
            "WAGE_REQUIRED");
      }
      ...
      if (hourlyRateCents < 0) {
        return createFailureResult(
            "Hourly rate " + formatCurrency(hourlyRateCents) + " is invalid - cannot be negative",
            "Enter a positive hourly rate between "
                + formatCurrency(MINIMUM_WAGE_CENTS)
                + " and "
                + formatCurrency(MAXIMUM_WAGE_CENTS),
            "WAGE_NEGATIVE");
      ...
      if (hourlyRateCents < MINIMUM_WAGE_CENTS) {
        return createFailureResult(
            "Hourly rate "
                + formatCurrency(hourlyRateCents)
                + " is outside valid range ("
                + formatCurrency(MINIMUM_WAGE_CENTS)
                + " - "
                + formatCurrency(MAXIMUM_WAGE_CENTS)
                + ")",
            "Verify hourly rate is within federal minimum wage ("
                + formatCurrency(MINIMUM_WAGE_CENTS)
                + ") and executive cap ("
                + formatCurrency(MAXIMUM_WAGE_CENTS)
                + ")",
  ...
  public FieldValidationResult validateFromString(String rateString) {
    if (rateString == null || rateString.trim().isEmpty()) {
      return createFailureResult(
          "Hourly rate is required and cannot be empty",
          "Enter hourly rate between "
              + formatCurrency(MINIMUM_WAGE_CENTS)
              + " and "
              + formatCurrency(MAXIMUM_WAGE_CENTS),
    ...
    try {
      ...
      return createFailureResult(
          "Hourly rate '" + rateString + "' is not a valid number",
          "Enter a numeric hourly rate between "
              + formatCurrency(MINIMUM_WAGE_CENTS)
              + " and "
              + formatCurrency(MAXIMUM_WAGE_CENTS),
          "WAGE_FORMAT_INVALID");
  ...
  public FieldValidationResult validateFromAnnualSalary(Integer annualSalary) {
    if (annualSalary == null || annualSalary <= 0) {
      return createFailureResult(
          "Annual salary must be positive for conversion",
