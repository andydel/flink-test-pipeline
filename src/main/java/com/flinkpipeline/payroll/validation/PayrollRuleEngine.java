...
/**
 * Payroll rule engine that orchestrates validation of employee records. Executes multiple
 * validation rules and aggregates results. Supports parallel execution for performance
 * optimization.
 */
public class PayrollRuleEngine {
  ...
  public PayrollValidationResult validate(PayrollEmployee employee) {
    ...
    try {
      ...
      FieldValidationResult errorField =
  ...
  private void addComplianceFlags(
      PayrollValidationResult result, List<FieldValidationResult> fieldResults) {
    boolean hasRegulatoryViolation =
        fieldResults.stream()
            .anyMatch(
                field ->
                    field.isFailed()
                        && field.getComplianceLevel()
    ...
    boolean hasPIIFields =
        fieldResults.stream()
            .anyMatch(
                field ->
                    "ssn".equals(field.getFieldName()) || "email".equals(field.getFieldName()));
